...
* The top rated book in Century `r params$centries` is `r round(max(centries_data$times.rated), 1)` 
for the book: **`r centries_data[centries_data$times.rated==max(centries_data$times.rated), 'titles']`**.

* The average rated book of the books by century `r params$centries` are equal `r round(mean(centries_data$times.rated), 1)`.

* The minimum rated book of the books by century `r params$centries` are equal `r round(min(centries_data$times.rated), 1)`.
...
* The maximum score for a book in Century `r params$centries` is `r round(max(centries_data$scores), 1)` for the book: **`r centries_data[centries_data$scores==max(centries_data$scores), 'titles']`**.

* The average score of all books in Century `r params$centries` is `r round(mean(centries_data$scores), 1)`.

* The minimum score for a book in Century `r params$centries` is `r round(min(centries_data$scores), 1)` for the book: **`r centries_data[centries_data$scores==min(centries_data$scores), 'titles']`**.
...
* The most prolific author in Century `r params$centries` is **`r unique centries_data[centries_data$books_written==max(centries_data$books_written), 'authors']`**.

* The least prolific author in Century `r params$centries` is **`r unique centries_data[centries_data$books_written==min(centries_data$books_written), 'authors']`**.
